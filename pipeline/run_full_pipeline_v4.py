...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run v4 pipeline: v1 extraction + hybrid normalization + synthesis, then analyze/group."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.82,
        help="Cosine similarity threshold for semantic hybrid normalization (default: 0.82).",
    )
    parser.add_argument(
        "--run-list-out",
    ...
    meta = {
        ...
        "run_dirs_v4": [str(p) for p in run_dirs],
    }
    (summary_dir / "meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
