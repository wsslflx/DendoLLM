...
for TC in "${TESTCASES[@]}"; do
    ...
    if python "$PIPELINE" --species-file "$SPECIES_FILE" --run-label "$TC" --bundle-root "$BATCH_ROOT" --shared-chroma-dir "$SHARED_CHROMA" --pdf-dir "$SHARED_PDF" --map-workers 4 --index-workers 4 --mapping-model "granite4.1:8b" --norm-batch-size 10 --force-reindex --force-reenrich >"$LOG_FILE" 2>&1; then
